"""
datautils.py - Unified data loading for UQ paper (4 PDEs)

Supports:
- 1D PDEs: .hdf5 format (Burgers, Advection)
- Diffusion-Reaction: .h5 format
- Navier-Stokes: .pt format
"""

import torch
import numpy as np
import h5py
from pathlib import Path
from torch.utils.data import Dataset, DataLoader, Subset


class PDEDataset(Dataset):
    """Memory-efficient dataset with in-place normalization and caching."""

    def __init__(self, X: np.ndarray, U: np.ndarray,
                 normalize_output: bool = True,
                 normalize_input: bool = True,
                 normalize_coords: bool = True,
                 cache_path: str = None):
        """
        Args:
            X: (n_samples, nx, ny, in_channels) - [x, y, input_field, ...]
            U: (n_samples, nx, ny, out_channels)
            normalize_output: Normalize U to N(0,1)
            normalize_input: Normalize input fields (beyond x,y) to N(0,1)
            normalize_coords: Normalize x,y to [-1, 1]
            cache_path: Path to save/load cached normalized dataset
        """
        self.n_samples, self.nx, self.ny, self.in_channels = X.shape
        self.normalize_coords = normalize_coords
        self.normalize_input = normalize_input
        self.normalize_output = normalize_output

        # Store normalization stats
        self.x_min = self.x_max = self.y_min = self.y_max = None
        self.x_fields_mean = []
        self.x_fields_std = []
        self.u_mean = 0.0
        self.u_std = 1.0

        # IN-PLACE normalization to save memory
        # Coordinate normalization
        if normalize_coords:
            self.x_min, self.x_max = X[..., 0].min(), X[..., 0].max()
            self.y_min, self.y_max = X[..., 1].min(), X[..., 1].max()
            # Normalize IN-PLACE
            X[..., 0] = 2 * (X[..., 0] - self.x_min) / (self.x_max - self.x_min + 1e-8) - 1
            X[..., 1] = 2 * (X[..., 1] - self.y_min) / (self.y_max - self.y_min + 1e-8) - 1

        # Input field normalization (channels 2+) IN-PLACE
        if normalize_input and self.in_channels > 2:
            for ch in range(2, self.in_channels):
                field = X[..., ch]
                mean, std = field.mean(), field.std()
                self.x_fields_mean.append(mean)
                self.x_fields_std.append(std)
                # Normalize IN-PLACE
                X[..., ch] = (field - mean) / (std + 1e-8)

        # Output normalization IN-PLACE
        if normalize_output:
            self.u_mean = U.mean()
            self.u_std = U.std()
            # Normalize IN-PLACE
            U[:] = (U - self.u_mean) / self.u_std

        # Convert to PyTorch tensors (X and U are already modified in-place)
        self.X = torch.from_numpy(X).float()
        self.U = torch.from_numpy(U).float()

        # Save cache if requested
        if cache_path:
            self.save_cache(cache_path)

    def __len__(self):
        return len(self.X)

    def __getitem__(self, idx):
        """
        Return data item. For 1D data (ny=1), only returns [x, input_field] channels.
        """
        X_item = self.X[idx]
        U_item = self.U[idx]

        # For 1D data (ny=1), extract only [x, input_field] (channels 0 and 2)
        # Skip channel 1 (dummy y coordinate)
        if self.ny == 1:
            X_item = torch.stack([X_item[..., 0], X_item[..., 2]], dim=-1)

        return X_item, U_item

    def denormalize(self, U_normalized: torch.Tensor) -> torch.Tensor:
        """Convert normalized predictions back to original scale."""
        if self.normalize_output:
            return U_normalized * self.u_std + self.u_mean
        return U_normalized

    def save_cache(self, cache_path: str):
        """Save normalized dataset to disk."""
        cache_data = {
            'X': self.X,
            'U': self.U,
            'n_samples': self.n_samples,
            'nx': self.nx,
            'ny': self.ny,
            'in_channels': self.in_channels,
            'x_min': self.x_min,
            'x_max': self.x_max,
            'y_min': self.y_min,
            'y_max': self.y_max,
            'x_fields_mean': self.x_fields_mean,
            'x_fields_std': self.x_fields_std,
            'u_mean': self.u_mean,
            'u_std': self.u_std,
            'normalize_coords': self.normalize_coords,
            'normalize_input': self.normalize_input,
            'normalize_output': self.normalize_output,
        }
        torch.save(cache_data, cache_path)
        print(f"Dataset cached to: {cache_path}")

    @staticmethod
    def load_cache(cache_path: str):
        """Load normalized dataset from disk."""
        cache_data = torch.load(cache_path, weights_only=False)

        # Create empty instance
        dataset = PDEDataset.__new__(PDEDataset)

        # Restore attributes
        dataset.X = cache_data['X']
        dataset.U = cache_data['U']
        dataset.n_samples = cache_data['n_samples']
        dataset.nx = cache_data['nx']
        dataset.ny = cache_data['ny']
        dataset.in_channels = cache_data['in_channels']
        dataset.x_min = cache_data['x_min']
        dataset.x_max = cache_data['x_max']
        dataset.y_min = cache_data['y_min']
        dataset.y_max = cache_data['y_max']
        dataset.x_fields_mean = cache_data['x_fields_mean']
        dataset.x_fields_std = cache_data['x_fields_std']
        dataset.u_mean = cache_data['u_mean']
        dataset.u_std = cache_data['u_std']
        dataset.normalize_coords = cache_data['normalize_coords']
        dataset.normalize_input = cache_data['normalize_input']
        dataset.normalize_output = cache_data['normalize_output']

        print(f"Dataset loaded from cache: {cache_path}")
        return dataset


def load_hdf5_1d(filepath: str, nu_values: list = None, max_samples: int = None,
                 time_step_spacing: int = 10, max_pairs_per_sample: int = 20) -> PDEDataset:
    """
    Load 1D PDE data (.hdf5): Burgers/Advection with multiple nu values.

    MEMORY-EFFICIENT VERSION: Uses chunked loading to prevent OOM.

    Args:
        filepath: Path to .hdf5 file
        nu_values: List of nu values to load (None = all)
        max_samples: Max samples per nu value
        time_step_spacing: Spacing between input and output time steps (default: 10)
        max_pairs_per_sample: Max number of pairs to extract per sample (None = all possible)

    Returns:
        PDEDataset
    """
    with h5py.File(filepath, 'r') as f:
        # PDEBench format: single 'tensor' with all nu values stacked
        if 'tensor' in f.keys():
            # Get shape WITHOUT loading data into memory
            data_shape = f['tensor'].shape  # (n_total, n_t, n_x)
            n_total, n_t, n_x = data_shape

            print(f"Dataset shape: {data_shape}")
            print(f"Estimated size: {n_total * n_t * n_x * 4 / (1024**3):.2f} GB")

            # Assume 4 nu values stacked: split into equal parts
            n_nu = 4  # PDEBench has 4 nu values for Burgers
            samples_per_nu = n_total // n_nu

            # Apply max_samples limit EARLY to reduce memory
            n_samples_to_load = samples_per_nu
            if max_samples is not None:
                n_samples_to_load = min(n_samples_to_load, max_samples)

            # Calculate total pairs estimate
            max_possible_pairs = (n_t - time_step_spacing)
            if max_pairs_per_sample is not None:
                pairs_per_sample = min(max_possible_pairs, max_pairs_per_sample)
            else:
                pairs_per_sample = max_possible_pairs

            total_pairs_estimate = n_nu * n_samples_to_load * pairs_per_sample
            print(f"Loading {n_samples_to_load} samples per nu value")
            print(f"Estimated total training pairs: {total_pairs_estimate}")

            # Pre-allocate arrays for better memory efficiency
            X_shape = (total_pairs_estimate, n_x, 1, 3)
            U_shape = (total_pairs_estimate, n_x, 1, 1)

            X_array = np.zeros(X_shape, dtype=np.float32)
            U_array = np.zeros(U_shape, dtype=np.float32)

            # Pre-compute coordinate grid (reuse for all samples)
            x_coords = np.linspace(0, 1, n_x)
            y_coords = np.array([0.5])
            grid_x, grid_y = np.meshgrid(x_coords, y_coords, indexing='ij')

            pair_idx = 0

            for nu_idx in range(n_nu):
                start_idx = nu_idx * samples_per_nu
                end_idx = start_idx + n_samples_to_load

                print(f"Loading nu group {nu_idx+1}/{n_nu}: samples {start_idx} to {end_idx}")

                # CHUNKED LOADING: Load only needed samples, not entire dataset
                nu_data = f['tensor'][start_idx:end_idx, :, :]  # (n_samples_to_load, n_t, n_x)

                # Extract multiple pairs per sample with configurable spacing
                for i in range(n_samples_to_load):
                    # Generate all possible pairs with given spacing
                    possible_pairs = []
                    for t_start in range(0, n_t - time_step_spacing):
                        t_end = t_start + time_step_spacing
                        if t_end < n_t:
                            possible_pairs.append((t_start, t_end))

                    # Limit number of pairs if specified
                    if max_pairs_per_sample is not None:
                        # Sample uniformly across the trajectory
                        step = max(1, len(possible_pairs) // max_pairs_per_sample)
                        possible_pairs = possible_pairs[::step][:max_pairs_per_sample]

                    # Create training pairs directly in pre-allocated array
                    for t_in, t_out in possible_pairs:
                        if pair_idx >= total_pairs_estimate:
                            break

                        input_field = nu_data[i, t_in, :]
                        output_field = nu_data[i, t_out, :]

                        # Store in 2D format for compatibility (will be squeezed for 1D FNO)
                        # X: (n_x, 1, 2) = [x, input_field] for 1D PDEs
                        X_array[pair_idx, :, :, 0] = x_coords[:, np.newaxis]  # x coordinates
                        X_array[pair_idx, :, :, 1] = grid_y  # dummy y (not used by FNO1d)
                        X_array[pair_idx, :, :, 2] = input_field[:, np.newaxis]

                        U_array[pair_idx, :, :, 0] = output_field[:, np.newaxis]

                        pair_idx += 1

                # Free memory
                del nu_data

            # Trim arrays to actual size
            X_array = X_array[:pair_idx]
            U_array = U_array[:pair_idx]

            print(f"Actual pairs loaded: {pair_idx}")
            print(f"Memory usage: X={X_array.nbytes / (1024**2):.2f} MB, U={U_array.nbytes / (1024**2):.2f} MB")

            return PDEDataset(X_array, U_array)

        else:
            # Old format with separate nu keys - use memory-efficient approach too
            all_nu = [k for k in f.keys() if k.startswith('nu_')]
            if nu_values is not None:
                all_nu = [k for k in all_nu if float(k.split('_')[1]) in nu_values]

            if not all_nu:
                raise ValueError(f"No nu keys found. Available: {list(f.keys())}")

            # First pass: count samples to pre-allocate
            total_pairs = 0
            for nu_key in all_nu:
                data_shape = f[nu_key].shape
                n_samples_nu = data_shape[0]
                if max_samples is not None:
                    n_samples_nu = min(n_samples_nu, max_samples)

                n_t = data_shape[1]
                max_possible_pairs = (n_t - time_step_spacing)
                if max_pairs_per_sample is not None:
                    pairs_per_sample = min(max_possible_pairs, max_pairs_per_sample)
                else:
                    pairs_per_sample = max_possible_pairs

                total_pairs += n_samples_nu * pairs_per_sample

            print(f"Estimated total training pairs: {total_pairs}")

            # Pre-allocate
            first_data = f[all_nu[0]]
            n_x = first_data.shape[2]

            X_array = np.zeros((total_pairs, n_x, 1, 3), dtype=np.float32)
            U_array = np.zeros((total_pairs, n_x, 1, 1), dtype=np.float32)

            # Pre-compute coordinate grid
            x_coords = np.linspace(0, 1, n_x)
            y_coords = np.array([0.5])
            grid_x, grid_y = np.meshgrid(x_coords, y_coords, indexing='ij')

            pair_idx = 0

            for nu_key in all_nu:
                print(f"Loading {nu_key}...")
                data = f[nu_key][:]  # (n_samples, n_t, n_x)
                n_samples, n_t, n_x = data.shape

                if max_samples is not None:
                    n_samples = min(n_samples, max_samples)
                    data = data[:n_samples]

                for i in range(n_samples):
                    # Generate all possible pairs with given spacing
                    possible_pairs = []
                    for t_start in range(0, n_t - time_step_spacing):
                        t_end = t_start + time_step_spacing
                        if t_end < n_t:
                            possible_pairs.append((t_start, t_end))

                    # Limit number of pairs if specified
                    if max_pairs_per_sample is not None:
                        step = max(1, len(possible_pairs) // max_pairs_per_sample)
                        possible_pairs = possible_pairs[::step][:max_pairs_per_sample]

                    # Create training pairs directly in pre-allocated array
                    for t_in, t_out in possible_pairs:
                        if pair_idx >= total_pairs:
                            break

                        input_field = data[i, t_in, :]
                        output_field = data[i, t_out, :]

                        X_array[pair_idx, :, :, 0] = grid_x
                        X_array[pair_idx, :, :, 1] = grid_y
                        X_array[pair_idx, :, :, 2] = input_field[:, np.newaxis]

                        U_array[pair_idx, :, :, 0] = output_field[:, np.newaxis]

                        pair_idx += 1

                # Free memory
                del data

            # Trim arrays to actual size
            X_array = X_array[:pair_idx]
            U_array = U_array[:pair_idx]

            print(f"Actual pairs loaded: {pair_idx}")
            print(f"Memory usage: X={X_array.nbytes / (1024**2):.2f} MB, U={U_array.nbytes / (1024**2):.2f} MB")

            return PDEDataset(X_array, U_array)


def load_h5_2d(filepath: str, max_samples: int = None) -> PDEDataset:
    """
    Load 2D diffusion-reaction data (.h5 format).

    Args:
        filepath: Path to .h5 file
        max_samples: Max samples to load

    Returns:
        PDEDataset
    """
    with h5py.File(filepath, 'r') as f:
        # Inspect keys
        keys = list(f.keys())

        # Find data key (usually simulation groups like '0000', '0001', ...)
        sim_keys = [k for k in keys if k.isdigit()]

        if not sim_keys:
            raise ValueError(f"No simulation keys found in {filepath}. Available: {keys}")

        X_list, U_list = [], []

        for sim_key in sim_keys:
            if max_samples and len(X_list) >= max_samples:
                break

            data = f[sim_key][:]  # (n_t, n_x, n_y, n_channels)
            n_t, n_x, n_y, _ = data.shape

            # Extract t=0 → t=-1
            input_field = data[0, :, :, 0]
            output_field = data[-1, :, :, 0]

            # Generate coordinate grid
            x_coords = np.linspace(0, 1, n_x)
            y_coords = np.linspace(0, 1, n_y)
            grid_x, grid_y = np.meshgrid(x_coords, y_coords, indexing='ij')

            # X: (n_x, n_y, 3)
            X = np.zeros((n_x, n_y, 3), dtype=np.float32)
            X[:, :, 0] = grid_x
            X[:, :, 1] = grid_y
            X[:, :, 2] = input_field

            # U: (n_x, n_y, 1)
            U = output_field[:, :, np.newaxis]

            X_list.append(X)
            U_list.append(U)

        X = np.stack(X_list)
        U = np.stack(U_list)

    return PDEDataset(X, U)


def load_pt_2d(filepath: str) -> PDEDataset:
    """
    Load Navier-Stokes data (.pt format).

    Args:
        filepath: Path to .pt file (expects dict with 'x' and 'y')

    Returns:
        PDEDataset
    """
    data = torch.load(filepath, weights_only=False)

    if not isinstance(data, dict) or 'x' not in data or 'y' not in data:
        raise ValueError(f"Expected dict with 'x' and 'y', got: {type(data)}")

    input_field = data['x'].numpy()  # (n_samples, n_x, n_y)
    output_field = data['y'].numpy()  # (n_samples, n_x, n_y)

    n_samples, n_x, n_y = input_field.shape

    # Generate coordinate grids
    x_coords = np.linspace(0, 1, n_x)
    y_coords = np.linspace(0, 1, n_y)
    grid_x, grid_y = np.meshgrid(x_coords, y_coords, indexing='ij')

    # X: (n_samples, n_x, n_y, 3)
    X = np.zeros((n_samples, n_x, n_y, 3), dtype=np.float32)
    X[:, :, :, 0] = grid_x[np.newaxis, :, :]
    X[:, :, :, 1] = grid_y[np.newaxis, :, :]
    X[:, :, :, 2] = input_field

    # U: (n_samples, n_x, n_y, 1)
    U = output_field[..., np.newaxis]

    return PDEDataset(X, U)


def load_ns_exp4(filepath: str, reynolds_numbers: list = None,
                 time_pairs: int = 5) -> tuple:
    """
    Load Navier-Stokes data for Experiment 4: OOD Detection.

    Format generated by datagen/navstokes.py:
    {
        'data': [
            {
                'velocity': (num_snapshots, 2, H, W),  # Velocity field snapshots
                'times': (num_snapshots,),
                'reynolds': float,
                'viscosity': float,
                'resolution': int,
                'dt': float
            },
            ...
        ],
        'metadata': {...}
    }

    Args:
        filepath: Path to .pt file
        reynolds_numbers: List of Reynolds numbers to load (None = all)
        time_pairs: Number of (input, output) pairs to create per sample

    Returns:
        PDEDataset with metadata containing Reynolds numbers for each sample
    """
    data_dict = torch.load(filepath, weights_only=False)

    if 'data' not in data_dict or 'metadata' not in data_dict:
        raise ValueError(f"Expected dict with 'data' and 'metadata' keys. Got: {data_dict.keys()}")

    all_data = data_dict['data']
    metadata = data_dict['metadata']

    print(f"Loading NS data from {filepath}")
    print(f"Available Reynolds numbers: {metadata.get('reynolds_numbers', 'unknown')}")
    print(f"Total samples in file: {len(all_data)}")

    X_list = []
    U_list = []
    reynolds_list = []

    for sample_idx, sample in enumerate(all_data):
        reynolds = sample['reynolds']

        # Filter by Reynolds number if specified
        if reynolds_numbers is not None and reynolds not in reynolds_numbers:
            continue

        velocity = sample['velocity']  # (num_snapshots, 2, H, W)
        num_snapshots, n_channels, H, W = velocity.shape

        # Convert to numpy if tensor
        if torch.is_tensor(velocity):
            velocity = velocity.numpy()

        # Compute velocity magnitude: |v| = sqrt(vx^2 + vy^2)
        vel_mag = np.sqrt(velocity[:, 0, :, :]**2 + velocity[:, 1, :, :]**2)  # (num_snapshots, H, W)

        # Generate coordinate grids
        x_coords = np.linspace(0, 1, H)
        y_coords = np.linspace(0, 1, W)
        grid_x, grid_y = np.meshgrid(x_coords, y_coords, indexing='ij')

        # Create time pairs for training: use snapshots as input->output pairs
        # Sample uniformly across the time series
        if time_pairs >= num_snapshots - 1:
            # Use all consecutive pairs
            pair_indices = [(i, i+1) for i in range(num_snapshots - 1)]
        else:
            # Sample uniformly
            step = max(1, (num_snapshots - 1) // time_pairs)
            pair_indices = [(i, i+1) for i in range(0, num_snapshots - 1, step)][:time_pairs]

        for t_in, t_out in pair_indices:
            input_field = vel_mag[t_in]   # (H, W)
            output_field = vel_mag[t_out]  # (H, W)

            # X: (H, W, 3) = [x, y, velocity_magnitude_t]
            X = np.zeros((H, W, 3), dtype=np.float32)
            X[:, :, 0] = grid_x
            X[:, :, 1] = grid_y
            X[:, :, 2] = input_field

            # U: (H, W, 1) = [velocity_magnitude_t+1]
            U = output_field[:, :, np.newaxis]

            X_list.append(X)
            U_list.append(U)
            reynolds_list.append(reynolds)

    # Stack into arrays
    X = np.stack(X_list, axis=0)  # (n_samples, H, W, 3)
    U = np.stack(U_list, axis=0)  # (n_samples, H, W, 1)
    reynolds_array = np.array(reynolds_list)  # (n_samples,)

    print(f"Loaded {len(X)} training pairs from {len(all_data)} simulations")
    print(f"Input shape: {X.shape}, Output shape: {U.shape}")

    # Create dataset
    dataset = PDEDataset(X, U)

    # Attach Reynolds numbers as metadata for OOD detection
    dataset.reynolds_numbers = reynolds_array

    return dataset, reynolds_array


def load_pde_data(filepath: str, pde_type: str, nu_values: list = None, max_samples: int = 8000,
                  time_step_spacing: int = 10, max_pairs_per_sample: int = 100) -> PDEDataset:
    """
    Unified loader for all 4 PDEs.

    Args:
        filepath: Path to data file
        pde_type: 'burgers', 'advection', 'diffusion_reaction', 'navier_stokes'
        nu_values: List of nu values (for 1D PDEs only)
        max_samples: Max samples to load
        time_step_spacing: Spacing between input and output time steps (1D PDEs only)
        max_pairs_per_sample: Max pairs to extract per sample (1D PDEs only)

    Returns:
        PDEDataset
    """
    filepath = Path(filepath)

    if pde_type in ['burgers', 'advection']:
        # 1D PDEs: .hdf5 format
        return load_hdf5_1d(str(filepath), nu_values, max_samples, time_step_spacing, max_pairs_per_sample)
    elif pde_type == 'diffusion_reaction':
        # 2D: .h5 format
        return load_h5_2d(str(filepath), max_samples)
    elif pde_type in ['navier_stokes', 'darcy']:
        # 2D: .pt format
        return load_pt_2d(str(filepath))
    else:
        raise ValueError(f"Unknown pde_type: {pde_type}")


def create_dataloaders(dataset: PDEDataset, train_ratio: float = 0.8,
                       val_ratio: float = 0.1, batch_size: int = 32,
                       seed: int = 42) -> tuple:
    """
    Split dataset and create dataloaders.

    Args:
        dataset: PDEDataset instance
        train_ratio: Fraction for training
        val_ratio: Fraction for validation
        batch_size: Batch size
        seed: Random seed

    Returns:
        train_loader, val_loader, test_loader
    """
    n_total = len(dataset)
    n_train = int(train_ratio * n_total)
    n_val = int(val_ratio * n_total)
    n_test = n_total - n_train - n_val

    # Random split
    indices = np.random.RandomState(seed=seed).permutation(n_total)
    train_idx = indices[:n_train]
    val_idx = indices[n_train:n_train+n_val]
    test_idx = indices[n_train+n_val:]

    train_dataset = Subset(dataset, train_idx)
    val_dataset = Subset(dataset, val_idx)
    test_dataset = Subset(dataset, test_idx)

    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=0)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False, num_workers=0)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, num_workers=0)

    return train_loader, val_loader, test_loader


def create_dataloaders_with_calibration(dataset: PDEDataset,
                                        train_ratio: float = 0.8,
                                        cal_ratio: float = 0.1,
                                        test_ratio: float = 0.1,
                                        batch_size: int = 32,
                                        seed: int = 42) -> tuple:
    """
    Create train/calibration/test data loaders for conformal prediction.

    This 3-way split is required for Conformal Prediction methods:
    - Train set: Used to train the base FNO model
    - Calibration set: Used to calibrate the conformal threshold
    - Test set: Used to evaluate coverage and interval width

    Args:
        dataset: PDEDataset instance
        train_ratio: Fraction for training (default: 0.8)
        cal_ratio: Fraction for calibration (default: 0.1)
        test_ratio: Fraction for test (default: 0.1)
        batch_size: Batch size
        seed: Random seed for reproducibility

    Returns:
        train_loader, cal_loader, test_loader

    Example:
        >>> dataset = load_pde_data('burgers.hdf5', 'burgers')
        >>> train_loader, cal_loader, test_loader = create_dataloaders_with_calibration(
        ...     dataset, train_ratio=0.8, cal_ratio=0.1, test_ratio=0.1
        ... )
        >>> # Train FNO on train_loader
        >>> # Calibrate conformal threshold on cal_loader
        >>> # Evaluate coverage on test_loader
    """
    # Validate ratios sum to 1.0
    assert abs(train_ratio + cal_ratio + test_ratio - 1.0) < 1e-6, \
        f"Ratios must sum to 1.0, got {train_ratio + cal_ratio + test_ratio}"

    n_total = len(dataset)
    n_train = int(n_total * train_ratio)
    n_cal = int(n_total * cal_ratio)
    n_test = n_total - n_train - n_cal

    # Random split with seed for reproducibility
    indices = np.random.RandomState(seed=seed).permutation(n_total)
    train_idx = indices[:n_train]
    cal_idx = indices[n_train:n_train+n_cal]
    test_idx = indices[n_train+n_cal:]

    # Create dataset subsets
    train_dataset = Subset(dataset, train_idx)
    cal_dataset = Subset(dataset, cal_idx)
    test_dataset = Subset(dataset, test_idx)

    # Create data loaders
    # Note: shuffle=True for train, shuffle=False for cal/test
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=0)
    cal_loader = DataLoader(cal_dataset, batch_size=batch_size, shuffle=False, num_workers=0)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, num_workers=0)

    print(f"Data split: Train={n_train} ({train_ratio*100:.1f}%), "
          f"Calibration={n_cal} ({cal_ratio*100:.1f}%), "
          f"Test={n_test} ({test_ratio*100:.1f}%)")

    return train_loader, cal_loader, test_loader


def create_kfold_splits(dataset: PDEDataset, k_folds: int = 5,
                        seed: int = 42) -> list:
    """
    Create K-fold cross-validation splits for Cross-Conformal Prediction.

    Used by Cross-Conformal method to train K models and aggregate calibration scores.

    Args:
        dataset: PDEDataset instance
        k_folds: Number of folds (default: 5)
        seed: Random seed

    Returns:
        List of (train_indices, val_indices) tuples for each fold

    Example:
        >>> folds = create_kfold_splits(dataset, k_folds=5)
        >>> for fold_idx, (train_idx, val_idx) in enumerate(folds):
        ...     train_subset = Subset(dataset, train_idx)
        ...     val_subset = Subset(dataset, val_idx)
    """
    n_total = len(dataset)
    indices = np.random.RandomState(seed=seed).permutation(n_total)

    fold_size = n_total // k_folds
    folds = []

    for i in range(k_folds):
        val_start = i * fold_size
        val_end = (i + 1) * fold_size if i < k_folds - 1 else n_total

        val_idx = indices[val_start:val_end]
        train_idx = np.concatenate([indices[:val_start], indices[val_end:]])

        folds.append((train_idx.tolist(), val_idx.tolist()))

    print(f"Created {k_folds} folds with ~{fold_size} samples per fold")
    return folds


def create_stratified_splits(dataset: PDEDataset,
                             stratify_fn,
                             batch_size: int = 32,
                             seed: int = 42) -> dict:
    """
    Create stratified data splits for Mondrian Conformal Prediction.

    Groups data by a stratification function and creates separate loaders per group.

    Args:
        dataset: PDEDataset instance
        stratify_fn: Function that takes (x, u) and returns group_id
        batch_size: Batch size
        seed: Random seed

    Returns:
        Dictionary mapping group_id -> DataLoader

    Example:
        >>> def stratify_by_magnitude(x, u):
        ...     mag = x[..., 2].abs().mean().item()
        ...     return 'low' if mag < 0.3 else 'medium' if mag < 0.7 else 'high'
        >>>
        >>> stratified_loaders = create_stratified_splits(dataset, stratify_by_magnitude)
        >>> # Returns {'low': loader1, 'medium': loader2, 'high': loader3}
    """
    from collections import defaultdict

    # Group samples by stratification criterion
    groups = defaultdict(list)

    for idx in range(len(dataset)):
        x, u = dataset[idx]
        group_id = stratify_fn(x, u)
        groups[group_id].append(idx)

    # Create loaders per group
    stratified_loaders = {}
    for group_id, indices in groups.items():
        subset = Subset(dataset, indices)
        loader = DataLoader(subset, batch_size=batch_size, shuffle=False, num_workers=0)
        stratified_loaders[group_id] = loader
        print(f"Group '{group_id}': {len(indices)} samples")

    return stratified_loaders


def create_ensemble_splits(dataset: PDEDataset,
                           n_models: int = 5,
                           bootstrap: bool = True,
                           train_ratio: float = 0.9,
                           batch_size: int = 32,
                           seed: int = 42) -> list:
    """
    Create training splits for Deep Ensemble with diverse initialization.

    Each ensemble member can be trained on:
    - Bootstrap sample (sample with replacement) if bootstrap=True
    - Different random split if bootstrap=False

    Args:
        dataset: PDEDataset instance
        n_models: Number of ensemble members
        bootstrap: Use bootstrap resampling (default: True)
        train_ratio: Fraction of data per model (default: 0.9)
        batch_size: Batch size
        seed: Random seed

    Returns:
        List of (train_loader, val_loader) tuples for each ensemble member

    Example:
        >>> ensemble_loaders = create_ensemble_splits(dataset, n_models=5)
        >>> for model_idx, (train_loader, val_loader) in enumerate(ensemble_loaders):
        ...     model = FNO2d(...)
        ...     train(model, train_loader, val_loader)
    """
    n_total = len(dataset)
    n_train = int(n_total * train_ratio)
    n_val = n_total - n_train

    ensemble_loaders = []

    for i in range(n_models):
        rng = np.random.RandomState(seed=seed + i * 1000)

        if bootstrap:
            # Bootstrap: sample with replacement
            train_idx = rng.choice(n_total, size=n_train, replace=True)
            # Validation: out-of-bag samples
            all_idx = set(range(n_total))
            train_set = set(train_idx)
            val_idx = np.array(list(all_idx - train_set))

            if len(val_idx) == 0:
                # Fallback if no OOB samples
                val_idx = rng.choice(n_total, size=n_val, replace=False)
        else:
            # Random split: sample without replacement
            indices = rng.permutation(n_total)
            train_idx = indices[:n_train]
            val_idx = indices[n_train:]

        train_subset = Subset(dataset, train_idx.tolist())
        val_subset = Subset(dataset, val_idx.tolist())

        train_loader = DataLoader(train_subset, batch_size=batch_size, shuffle=True, num_workers=0)
        val_loader = DataLoader(val_subset, batch_size=batch_size, shuffle=False, num_workers=0)

        ensemble_loaders.append((train_loader, val_loader))

    print(f"Created {n_models} ensemble splits ({'bootstrap' if bootstrap else 'random'})")
    return ensemble_loaders


def create_ood_test_data(dataset: PDEDataset,
                         ood_type: str = 'gaussian_noise',
                         noise_level: float = 0.1,
                         test_ratio: float = 0.2,
                         batch_size: int = 32,
                         seed: int = 42):
    """
    Create out-of-distribution (OOD) test data for robustness evaluation.

    Tests all UQ methods' ability to detect and quantify uncertainty on corrupted inputs.

    Args:
        dataset: PDEDataset instance
        ood_type: Type of corruption
            - 'gaussian_noise': Add Gaussian noise to inputs
            - 'uniform_noise': Add uniform noise
            - 'salt_pepper': Salt-and-pepper noise
            - 'blur': Gaussian blur (for 2D)
            - 'zero_out': Randomly zero out regions
        noise_level: Corruption strength (default: 0.1)
        test_ratio: Fraction of data to corrupt (default: 0.2)
        batch_size: Batch size
        seed: Random seed

    Returns:
        ood_loader: DataLoader with corrupted inputs
        clean_loader: DataLoader with clean inputs (same samples for comparison)

    Example:
        >>> ood_loader, clean_loader = create_ood_test_data(
        ...     dataset, ood_type='gaussian_noise', noise_level=0.5
        ... )
        >>> # Evaluate: uncertainty should be higher on OOD data
    """
    n_total = len(dataset)
    n_test = int(n_total * test_ratio)

    # Select test samples
    rng = np.random.RandomState(seed=seed)
    test_idx = rng.choice(n_total, size=n_test, replace=False)

    # Get clean data
    X_clean = []
    U_clean = []
    for idx in test_idx:
        x, u = dataset[idx]
        X_clean.append(x)
        U_clean.append(u)

    X_clean = torch.stack(X_clean)
    U_clean = torch.stack(U_clean)

    # Create corrupted version
    X_ood = X_clean.clone()

    if ood_type == 'gaussian_noise':
        # Add Gaussian noise to input fields (not coordinates)
        noise = torch.randn_like(X_ood[..., 2:]) * noise_level
        X_ood[..., 2:] = X_ood[..., 2:] + noise

    elif ood_type == 'uniform_noise':
        noise = (torch.rand_like(X_ood[..., 2:]) - 0.5) * 2 * noise_level
        X_ood[..., 2:] = X_ood[..., 2:] + noise

    elif ood_type == 'salt_pepper':
        # Randomly set pixels to min/max values
        mask = torch.rand_like(X_ood[..., 2:]) < noise_level
        X_ood[..., 2:][mask] = torch.where(
            torch.rand_like(X_ood[..., 2:])[mask] > 0.5,
            X_ood[..., 2:].max(),
            X_ood[..., 2:].min()
        )

    elif ood_type == 'zero_out':
        # Randomly zero out regions
        mask = torch.rand_like(X_ood[..., 2:]) < noise_level
        X_ood[..., 2:][mask] = 0

    elif ood_type == 'blur':
        # Simple box blur for 2D data
        if X_ood.dim() == 4:  # (batch, nx, ny, channels)
            kernel_size = max(3, int(noise_level * 10))
            # Apply blur to input field channels
            from torch.nn.functional import avg_pool2d
            for c in range(2, X_ood.shape[-1]):
                field = X_ood[..., c].unsqueeze(1)  # (batch, 1, nx, ny)
                blurred = avg_pool2d(field, kernel_size, stride=1, padding=kernel_size//2)
                X_ood[..., c] = blurred.squeeze(1)

    else:
        raise ValueError(f"Unknown ood_type: {ood_type}")

    # Create datasets
    class SimpleDataset(Dataset):
        def __init__(self, X, U):
            self.X = X
            self.U = U
        def __len__(self):
            return len(self.X)
        def __getitem__(self, idx):
            return self.X[idx], self.U[idx]

    ood_dataset = SimpleDataset(X_ood, U_clean)  # Corrupted input, clean output
    clean_dataset = SimpleDataset(X_clean, U_clean)

    ood_loader = DataLoader(ood_dataset, batch_size=batch_size, shuffle=False, num_workers=0)
    clean_loader = DataLoader(clean_dataset, batch_size=batch_size, shuffle=False, num_workers=0)

    print(f"Created OOD test set: {n_test} samples with {ood_type} (level={noise_level})")

    return ood_loader, clean_loader


def get_calibration_dataset(dataset: PDEDataset,
                            indices: np.ndarray) -> Dataset:
    """
    Get calibration dataset as a Dataset (not DataLoader) for methods that need direct access.

    Used by Full Conformal (LOO) and Cross-Conformal which need to iterate over individual samples.

    Args:
        dataset: PDEDataset instance
        indices: Indices for calibration samples

    Returns:
        Calibration dataset (Subset)

    Example:
        >>> # For Full Conformal LOO
        >>> cal_dataset = get_calibration_dataset(dataset, cal_indices)
        >>> for i in range(len(cal_dataset)):
        ...     x_i, y_i = cal_dataset[i]  # Access individual samples
    """
    return Subset(dataset, indices.tolist() if isinstance(indices, np.ndarray) else indices)


def create_subset_loader(dataset: PDEDataset,
                        n_samples: int,
                        batch_size: int = 32,
                        seed: int = 42) -> DataLoader:
    """
    Create a DataLoader with a specific number of samples from the dataset.

    Used for Experiment 3 (Epistemic vs Aleatoric) to train models on varying data sizes
    while keeping a consistent test set.

    Args:
        dataset: PDEDataset instance
        n_samples: Number of samples to include in the subset
        batch_size: Batch size for the DataLoader
        seed: Random seed for reproducibility

    Returns:
        DataLoader with n_samples from the dataset

    Example:
        >>> # Create loaders with different training sizes
        >>> train_100 = create_subset_loader(dataset, n_samples=100, seed=42)
        >>> train_500 = create_subset_loader(dataset, n_samples=500, seed=42)
        >>> # Train separate models on each subset
    """
    n_total = len(dataset)

    if n_samples > n_total:
        raise ValueError(f"Requested {n_samples} samples but dataset only has {n_total}")

    # Random sample indices
    rng = np.random.RandomState(seed=seed)
    indices = rng.choice(n_total, size=n_samples, replace=False)

    # Create subset
    subset = Subset(dataset, indices.tolist())

    # Create DataLoader
    loader = DataLoader(subset, batch_size=batch_size, shuffle=True, num_workers=0)

    print(f"Created subset loader: {n_samples} samples (batch_size={batch_size})")

    return loader
