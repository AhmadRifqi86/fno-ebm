...
\textbf{Explanation}: The quadratic error term $(u^* - \gamma)^2$ creates stronger penalties for large errors. 
...
This scheme uses error-magnitude-dependent weighting that automatically scales regularization:
\begin{equation}
    \mathcal{L}_{\text{adaptive}} = \mathcal{L}_{\text{NIG}} + \lambda \cdot \left(e^{|u^* - \gamma|} - 1\right) \cdot (2\nu + \alpha)
\end{equation}

\textbf{Explanation}: The exponential term $(e^{|u^* - \gamma|} - 1)$ creates an adaptive weight that grows
exponentially with prediction error. For small errors, $e^{\epsilon} - 1 \approx \epsilon$, providing gentle
regularization. For large errors, the exponential amplifies the penalty, strongly discouraging high evidence
on incorrect predictions. This creates automatic error-dependent scaling without requiring manual tuning.
...
\textbf{Explanation}: Unlike L1 regularization which encourages sparsity, L2 regularization on $\nu^2$ creates 
...
\begin{multline}
    ...
    + \lambda_{\text{KL}} \cdot D_{\text{KL}}\left(\text{NIG}(\gamma, \nu, \alpha, \beta) \| \text{NIG}_0\right)
\end{multline}

The KL divergence between NIG distributions has closed form:
\begin{multline}
    D_{\text{KL}} = \frac{1}{2}\left[\frac{\nu_0}{\nu} - \log\frac{\nu_0}{\nu} - 1\right] + \alpha_0\log\frac{\beta}{\beta_0} \\
    - \log\frac{\Gamma(\alpha)}{\Gamma(\alpha_0)} + (\alpha - \alpha_0)\psi(\alpha) + \frac{\beta_0 - \beta}{\beta}\alpha
\end{multline}
where $\psi(\cdot)$ is the digamma function and $(\gamma_0, \nu_0, \alpha_0, \beta_0)$ are prior hyperparameters.

\textbf{Explanation}: This is the most principled Bayesian approach. The KL term acts as a prior, pulling the 
