...
In standard regression, a neural network predicts a single value $\hat{y}$ for each input. Evidential regression instead predicts parameters of the Normal-Inverse-Gamma (NIG) distribution (defined in Section~\ref{sec:preliminaries}) that represents uncertainty over both the prediction mean and variance.
...
At each spatial location $(i,j)$, the four NIG parameters $(\gamma_{i,j}, \nu_{i,j}, \alpha_{i,j}, \beta_{i,j})$ define a distribution over Gaussian likelihood parameters, providing closed-form expressions for predictive statistics:
\begin{align}
...
The epistemic and aleatoric uncertainty decomposition follows directly from the NIG parameterization (Section~\ref{sec:preliminaries}). The epistemic uncertainty is inversely proportional to $\nu$, representing the ``evidence'' or effective number of virtual observations. Regions where the model has seen similar training data will have higher $\nu$ and thus lower epistemic uncertainty.
