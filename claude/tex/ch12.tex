...
Neural operators have emerged as a powerful paradigm for learning solution operators of partial differential 
equations (PDEs), offering orders of magnitude speedup compared to traditional numerical solvers while maintaining 
competitive accuracy. Among neural operator architectures, Fourier Neural Operators (FNOs) \cite{li2020fourier} 
have demonstrated remarkable success across diverse applications including fluid dynamics, climate modeling, and 
materials science. However, the deterministic nature of standard FNOs presents a critical limitation: they 
provide point predictions without quantifying prediction uncertainty, making them unsuitable for safety-critical 
applications or decision-making under uncertainty.

Uncertainty quantification (UQ) for neural networks has been extensively studied in the computer vision and natural 
language processing communities, with several prominent approaches emerging: Monte Carlo 
Dropout \cite{gal2016dropout}, Deep Ensembles \cite{lakshminarayanan2017simple}, Bayesian Neural Networks 
\cite{blundell2015weight}, and more recently, Evidential Deep Learning \cite{sensoy2018evidential,amini2020deep}. 
However, the application of these methods to neural operators remains understudied, with most existing work 
focusing on point prediction accuracy rather than uncertainty quantification. This gap is particularly concerning 
given that many PDE-solving applications—such as weather forecasting, drug discovery, and engineering 
design—critically require reliable uncertainty estimates.
...
\begin{enumerate}
    ...
    \item \textbf{RQ3 (Regularization):} For evidential methods, how do different regularization schemes (standard \cite{amini2020deep}, improved \cite{oh2022improving}, uncertainty-aware \cite{ye2024uncertainty}, Adaptive, $L^2$ evidence, KL-divergence) affect calibration and uncertainty quality?
...
Uncertainty quantification aims to not only predict outputs but also quantify the confidence in those predictions. In the Bayesian framework \cite{bishop2006pattern,murphy2012machine}, we seek the posterior predictive distribution:
\begin{equation}
...
Uncertainty is commonly decomposed into two types \cite{kendall2017uncertainties,der2009aleatory}:
\begin{itemize}
...
We evaluate UQ methods using metrics that assess predictive accuracy, calibration quality, and uncertainty informativeness:

\subsubsection{Predictive Accuracy Metrics}

\textbf{Mean Squared Error (MSE):} Measures average squared prediction error:
\begin{equation}
    \text{MSE} = \frac{1}{N}\sum_{i=1}^{N} \|y_i - \hat{y}_i\|^2
\end{equation}
where $y_i$ is the true output and $\hat{y}_i$ is the predicted mean. Lower is better.

\textbf{Mean Absolute Error (MAE):} Measures average absolute prediction error:
\begin{equation}
    \text{MAE} = \frac{1}{N}\sum_{i=1}^{N} \|y_i - \hat{y}_i\|
\end{equation}
Lower is better; less sensitive to outliers than MSE.

\textbf{Relative $L^2$ Error:} Normalized prediction error:
\begin{equation}
    \text{rel\_l2} = \frac{\|y - \hat{y}\|_2}{\|y\|_2}
\end{equation}
Accounts for output magnitude; useful for comparing across different PDEs.

\subsubsection{Calibration Metrics}

\textbf{Expected Calibration Error (ECE):} Measures alignment between predicted uncertainty and actual errors \cite{guo2017calibration}. Predictions are binned by uncertainty, and ECE computes the average gap between confidence and accuracy across bins. Lower is better (perfect calibration: ECE = 0).

\textbf{Coverage:} Fraction of test samples whose true values fall within predicted $\alpha$-confidence intervals:
\begin{equation}
...
\end{equation}
Target: 0.95 for 95\% confidence intervals (i.e., $\alpha = 0.95$, $z_\alpha = 1.96$).

\textbf{Interval Width:} Average width of prediction intervals:
\begin{equation}
    \text{Width} = \frac{1}{N}\sum_{i=1}^{N} 2z_\alpha\hat{\sigma}_i
\end{equation}
Lower is better (sharper), but coverage must be maintained.

\subsubsection{Uncertainty Quality Metrics}

\textbf{Uncertainty-Error Correlation:} Pearson correlation between predicted uncertainty $\hat{\sigma}_i$ and actual absolute error $|y_i - \hat{y}_i|$:
\begin{equation}
    \rho = \text{corr}(\hat{\sigma}, |y - \hat{y}|)
\end{equation}
Higher is better; indicates uncertainty is informative about prediction quality.

\textbf{Negative Log-Likelihood (NLL):} Probabilistic metric under Gaussian assumption:
\begin{equation}
    \text{NLL} = -\frac{1}{N}\sum_{i=1}^{N} \log p(y_i|\hat{y}_i, \hat{\sigma}_i^2) = \frac{1}{2N}\sum_{i=1}^{N} \left[\log(2\pi\hat{\sigma}_i^2) + \frac{(y_i - \hat{y}_i)^2}{\hat{\sigma}_i^2}\right]
\end{equation}
Higher (less negative) is better; jointly evaluates accuracy and calibration.

\textbf{AUROC for OOD Detection:} Area under ROC curve for classifying in-distribution vs. out-of-distribution samples using predicted uncertainty as the decision score. 0.5 = random, 1.0 = perfect detection.

Chapter 3 provides implementation details for computing these metrics on neural operator predictions.