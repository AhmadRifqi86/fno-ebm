...
\item $\sigma$ is the activation function (GELU \cite{hendrycks2016gaussian})
\end{itemize}
...
The FNO's key advantage is discretization-invariance \cite{li2020fourier}: once trained on a $64 \times 64$ grid, it can be evaluated on any resolution without retraining, as the Fourier integral operator is defined on continuous functions.
...
\begin{itemize}
\item Dropout type: Dropout2d (spatial dropout \cite{tompson2015efficient} for 2D data)
...
\item Optimizer: Adam \cite{kingma2014adam} with learning rate $\eta = 10^{-3}$
\item Epochs: 20
...
We implement six variants of evidential deep learning with different regularization schemes, all based on the Normal-Inverse-Gamma (NIG) parameterization \cite{murphy2012machine}. The FNO predicts four parameters $(\gamma, \nu, \alpha, \beta)$ for each spatial location, which define a NIG distribution over the Gaussian likelihood parameters:
...
The predictive distribution is a Student's t-distribution \cite{murphy2012machine} with:
\begin{itemize}
...
\begin{itemize}
\item \textbf{Physical system}: 2D incompressible viscous flow \cite{anderson1995computational}
...
\begin{itemize}
\item \textbf{Physical system}: Steady-state flow through porous media \cite{darcy1856}
...
\section{Experimental Protocol} %mybe change to experiment design
...
\begin{itemize}
\item \textbf{Methods}: 6 evidential variants + Ensemble + MC Dropout (8 total)
\item \textbf{Data}: Darcy Flow (5000 train, 1000 test)
\item \textbf{Metrics}: All 8 metrics (MSE, MAE, rel\_l2, ECE, Coverage, Interval Width, Correlation, NLL)
\item \textbf{Goal}: Evaluate whether performance rankings from Navier-Stokes generalize to different physics (linear, steady-state flow through porous media)
\end{itemize}

\subsection{Experiment 3: Epistemic-Aleatoric Decomposition}
...
\subsection{Experiment 4: Parameter Extrapolation OOD}
...
\subsection{Experiment 5: Cross-PDE OOD}
...
\subsection{Experiment 6: Architectural Ablation Study}

\begin{itemize}
\item \textbf{Methods}: Baseline FNO + 7 ablation variants
\item \textbf{Data}: Navier-Stokes 2D (7000 train, 1500 val, 1500 test)
\item \textbf{Ablations}: Remove skip connections, remove activations, shallow heads, linear heads, add batch norm, Fourier-only, add residual connection
\item \textbf{Metrics}: MSE, MAE, rel\_l2, calibration error, uncertainty-error correlation
\item \textbf{Goal}: Identify critical architectural components for evidential FNO performance
\end{itemize}

% \section{Implementation Details}

% All experiments were implemented in PyTorch 2.0 \cite{paszke2019pytorch}. Training was performed on NVIDIA A100 GPUs with mixed precision (FP16) \cite{micikevicius2017mixed}. Batch size was set to 16 for all methods. No data augmentation was used. All models were trained for 20 epochs with early stopping based on validation loss (patience = 5 epochs).